...
//         Phone notDoable = new MerchandiseV2();
        ...
        // >> TODO ph和 m都指向同一个对象，但是通过 m就不可以调用getBrand方法
        //    TODO 因为m的类型是MerchandiseV2，MerchandiseV2里没有你定义getBrand方法
        // m.getBrand();

        // TODO 如果确定一个父类的引用指向的对象，实际上就是一个子类的对象（或者子类的子类的对象），可以强制类型转换
        Phone aPhone = (Phone) m2;//m2是MerchandiseV2类型的，Phone是MerchandiseV2的子类，可以强制把父类转换成子类
 ...
 */
        // TODO 有点绕，后续继续看
        // TODO 会出错，因为 m2指向的是一个Phone类型的对象，不是ShellColorChangePhone的对象
